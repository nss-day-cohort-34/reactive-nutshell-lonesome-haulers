    ...
    handleLogin = (event) => {
        ...
        UserManager.getUsernamePassword(this.state.username, this.state.password).then(user => {
            if (user.length === 0) {
                ...
                this.setState({id: user[0].id, color: user[0].color, backgroundColor: user[0].backgroundColor})
    ...
    render() {
        return (
            ...
            <form onSubmit={this.handleLogin}>
                ...
                <img className="login_img" src={ require('./Nutshell_logo.png') } alt="Nutshell logo"/>
                <fieldset>
                    <h3>Login</h3>
